...
const TabsPostDetail: React.FC = () => {
  ...
  const tabItems = useMemo(
    () => [
      { label: "Maison Another House", slug: "all" },
  ...
  const activeLabel = useMemo(() => {
    return tabItems.find((item) => item.slug === activeTab)?.label || "Maison Another House";
  }, [activeTab, tabItems]);
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex flex-col items-center justify-center w-full text-center">
        <div className="text-2xl md:text-[52px] font-medium text-gray-900 py-16">
          <div className="text-lg md:text-[22px]">Maison Another House</div>
          {activeLabel}
        </div>
        <div
          ref={tabsRef}
          className={`w-full ${isSticky ? "fixed top-0 left-0 right-0 z-50 bg-white shadow-md py-4" : ""}`}
        >
          {isLoading ? (
